...
class NewsScoreService:
    ...
    async def save_today(self, payload: NewsScoreIn) -> NewsScoreOut:
        ...
        logger.info(
            "News score saved: %s slot=%d%s, score=%.1f (%s)",
            record.score_date,
            record.slot,
    ...
    async def delete_slot(self, slot: int) -> NewsScoreOut:
        """撤销当日某一次打分，释放该槽位。"""
        if not 1 <= slot <= MAX_DAILY_SLOTS:
            raise ValueError(f"槽位需在 1~{MAX_DAILY_SLOTS} 之间")
        removed = await self._repo.delete_slot(date.today(), slot)
        if not removed:
            raise ValueError(f"今日第 {slot} 次打分不存在，无法撤销")
        logger.info("News score deleted: %s slot=%d", date.today(), slot)
