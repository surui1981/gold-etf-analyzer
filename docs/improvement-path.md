...
> 当前版本：**V0.52.0** ｜ 制定日期：2026-08-31 ｜ 状态：**P0-P2 已全部落地；6.1 数据时效透明已落地** ｜ 文档更新：2026-09-04（6.1 落地 + 版本 bump）
...
| **V0.52.0**（当前） | **6.1 数据时效透明**：全局时效条 + 三市场时段判定 + 降级持续角标；启动期 Alembic 改为子进程执行（彻底消除迁移死锁） | 数据可信 / 启动可靠 | ✅ |
| **V0.52.0** | 6.1 数据时效透明：全局时效条 + 交易时段判定 + 降级持续角标 | UX 信任 | ✅ 已落地 |
...
> 基于 V0.52.0 现状，围绕「数据可信、看得懂、用得顺、记得住、主动提醒」五个体验目标梳理可落地方向。
