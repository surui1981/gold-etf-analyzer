...
> 当前版本：**V0.53.0** ｜ 制定日期：2026-08-31 ｜ 状态：**P0-P2 已全部落地；6.1 数据时效透明 + 6.2 响应式适配已落地** ｜ 文档更新：2026-09-04（6.2 响应式落地 + 版本 bump）
...
| **V0.53.0**（当前） | **6.2 响应式与移动端适配**：新增共享 `static/responsive.css`（768/480/360 三档断点）并在四页注入，窄屏下顶栏换行、面板与导航纵向堆叠、固定宽列收窄、表单输入满宽、持仓表格容器内横向滚动、按钮触控放大 | 多端可用 | ✅ |
