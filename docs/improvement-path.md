...
> 当前版本：**V0.59.0** ｜ 制定日期：2026-08-31 ｜ 状态：**P0-P2 已全部落地 + P1 #5 行情源配置化（V0.59.0）；6.1 数据时效透明 + 6.2 响应式适配 + 6.3 决策可解释性 + 6.4 操作防错与撤销 + 6.6 主动提醒 + 6.8 新手引导与帮助体系（V0.58.0）+ 6.10 央行购金数据化与自动化（V0.57.0）+ P1 #5 行情源配置化（V0.59.0）已落地**（6.5 个性化 / 6.7 多时间框架 / 6.9 离线体验 待规划） ｜ 文档更新：2026-09-11（V0.59.0 行情源配置化 + 版本 bump）
