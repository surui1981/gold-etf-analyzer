...
> 当前版本：**V0.54.0** ｜ 制定日期：2026-08-31 ｜ 状态：**P0-P2 已全部落地；6.1 数据时效透明 + 6.2 响应式适配 + 6.3 决策可解释性已落地** ｜ 文档更新：2026-09-04（6.3 决策可解释性落地 + 版本 bump）
...
| **V0.54.0**（当前） | **6.3 决策可解释性增强**：决策理由结构化（利多/利空中性三向 `reason_items`），前端以红绿对照条呈现；指数构成（技术/宏观/消息）堆叠条可视化；保留 `reasons` 向后兼容 | 决策看得懂 | ✅ |
