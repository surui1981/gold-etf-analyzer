...
> 项目名：`gold-etf-analyzer` ｜ 当前版本：**V0.51.1**
...
| **V0.51.1**（当前） | **启动健壮性修复**：移除 Alembic 前的冗余 `create_all`，消除 SQLite 双引擎争写锁导致启动 hang；新增 WAL checkpoint（防异常退出遗留锁）+ 迁移 30s 超时兜底；启动后后台预热行情缓存使首屏直接命中；前端 `fetch` 增加 40s 超时与失败可见态，避免页面静默空白 | 启动/加载可靠性 |
