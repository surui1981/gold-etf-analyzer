...
> 项目名：`gold-etf-analyzer` ｜ 当前版本：**V0.20**
...
| **V0.20**（当前） | **三面评估体系**（技术 30%/宏观 40%/消息面 30%）、**消息面评估页**（客户投行展望打分化）、**权重配置页**（持久化）、**每日评估快照**（daily_snapshots 本地历史）、宏观参考因子（美元/美债/VIX/央行购金）、**仓位推荐**（评估指数→建议仓位）、数据源 30s 超时兜底、统一顶部导航、投资警示五条、本机常驻（start_server.bat 一键自愈 + install_startup.ps1）、56 测试 |
...
| # | 事项 | 状态 | 价值 |
|---|------|------|------|
| 1 | 发布 **V0.20**（commit + `v0.20.0` 标签推送 GitHub） | ✅ | 固化三面体系/消息面/权重配置/每日快照/仓位推荐 |
