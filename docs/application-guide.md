...
| **可观测性与数据守门** | `X-Request-ID` 全链路追踪：纯 ASGI `TraceIdMiddleware` 入站沿用或 UUIDv4 hex 自动生成、contextvars 进程内隔离、响应头回写、长度 8-128 字符 alnum+-._ 防日志注入；日志格式器自动附加 trace_id（`时间 \| 级别 \| trace_id \| logger \| 消息`）；价格日历 `upsert_many` 入库前 schema 校验（`close > 0` 拒 NaN/0/负、`source` 白名单 `{live, manual, import, test}`、单日涨跌幅超 ±50% 跳过该条目其余正常）；GitHub Actions CI（Python 3.11/3.12 matrix + uv 缓存 + concurrency 取消旧 PR + `fail-fast: false`） | ✅ V0.67.0 |
| **导航折叠 + 全局搜索 + 客户端埋点底座** | 移动端 ≤768px 顶栏自动折叠为汉堡抽屉（`nav-drawer.js` 自注入，复制 .links 元素到抽屉）；桌面 ⌘K / Ctrl+K 唤起全局命令面板（`command-palette.js`，13 命令：7 页导航 + 刷新 + 时间区间 1D/5D/1M + 主题切换预埋 + 帮助重看，↑↓/Enter/Esc）；前端 `telemetry.js` 批量上报（sendBeacon 优先 + fetch keepalive 兜底，4s/20 条 flush + visibilitychange + 全局 error 兜底）；`POST /api/v1/telemetry/ingest`（白名单 10 类事件：page_view / action_click / range_change / error_caught / palette_open / palette_query / palette_select / nav_drawer_open / nav_drawer_select / theme_change，page 必须 `/` 开头，payload ≤50 字段）+ `GET /api/v1/telemetry/stats?days=N`（按类型 24h/N 天聚合 + 占比）；7 页统一注入 | ✅ V0.68.0 |
...
| **行情源 provider 可切换** | `.env` 配置 `MARKET_PROVIDER=akshare\|mock\|eastmoney_only\|sina_only\|silver_yahoo`，5 选 1；`market_providers.py` 工厂解析 bundle 注入；XAU fallback chain 与缓存 TTL 也可配；旧 `provider=` 签名保留向后兼容；`silver_yahoo` 为 V0.73.0 N+12 新增白银 Yahoo Finance provider | ✅ V0.59.0（V0.73.0 N+12 扩展） |
...
| **i18n 国际化（V0.73.0 P3-c #17）** | `static/i18n.js` IIFE（`t/setLang/apply/fmt.{number,currency,date,dateTime,time,percent,relative}` 11 个格式化方法，基于 `Intl.NumberFormat` / `Intl.DateTimeFormat`）+ 三语字典 `static/i18n/{zh-CN,zh-TW,en-US}.js`（V0.73.0 时 zh-CN 626 / en-US 627 / zh-TW 391 个 key；**V0.74.0 实测 699 / 700 / 451，zh-TW 覆盖率 64.5%**）+ 顶栏 `<select class="lang-sel">` 切换器（持久化 `localStorage.pm_lang`）+ FOUC guard inline head script；① **PR-N+7** i18n 框架 + portfolio 英文版；② **PR-N+8** 9 页英文版全覆盖；③ **PR-N+9** zh-TW 字典 33→391 key（country.* 33 + 6 页 chrome + fresh.* 13，覆盖率 10.9%→61.2%）+ `static/freshness.js` 全本地化（`fmtAge` → `I18n.fmt.relative()` + `fresh.*` 字典 13 key 三语）+ 后端 `schemas/central_bank.py` `country_name: str \| None = None`（DB 列保留向后兼容）+ `services/central_bank.py` `_resolve_country_name()` 兜底链（DB → `COUNTRY_NAMES[iso]` → iso，永不返回 None）+ 前端 `static/central_bank.html` 渲染链 `I18n.t('country.' + iso) → country_name → iso` + 新增 `escapeHtml()` helper 防 XSS；④ **测试** `tests/test_i18n/test_i18n_format.py`（15）+ `tests/test_i18n/test_i18n_coverage.py`（7）+ 阈值 10%→60%，基线 621 → **644**，目标分 91 → **91.5** | ✅ V0.73.0 |
| **i18n.apply() inline 子节点保留（V0.73.0 N+11）** | `static/i18n.js` `apply()` 重写：原 `el.textContent = t(...)` 会一并擦掉 inline 子元素（`<span id="badge">` / `<button id="refreshBtn">` / `<span id="clSum">`），导致后续 `getElementById` 拿到 null 报错；改为遍历 `Array.from(el.childNodes)`：① 首个文本节点 → 替换 `nodeValue`；② 后续文本节点 → `removeChild`；③ 无文本节点时 → `insertBefore(document.createTextNode(...))` 前插；attribute key（placeholder / title / aria / html）路径不受影响；新增 `tests/test_i18n/test_i18n_apply.py` 5 例覆盖（单文本节点 / 文本+inline 子 / 仅 inline 子前插 / 多文本节点合并 / attribute key 不受影响） | ✅ V0.73.0 N+11 |
...
| **V0.69.0** | **主题切换 + 无障碍扩面**：① 4 主题（light / dark / auto / **high-contrast**）—— `static/themes.css` 统一 CSS 变量；用户偏好持久化 `localStorage.pm_theme` + `prefers-color-scheme` 监听；② Chart.js canvas 加 `role="img"` + `aria-label` + 数据表 fallback（点击展开）；③ 全站 axe-core 0 critical + 键盘 Tab 序修复 + skip-to-content 链接；④ 主题切换埋点 `theme_change` 接入 | 无障碍扩面 | ✅ |
...
>
> **下一阶段路线（V0.67.0 → V0.72.0）**：见 [improvement-path.md](improvement-path.md) **§三·五**——基于 2026-09-16 工程性评估（综合 **81.5 / 100 B+**，数据 82 / 框架 78 / 易用性 85）排序的工程优先级（CI/CD → trace_id → Prometheus → Service Worker → 主题 / 无障碍 → 共振信号 → 克数持仓 → 多品种白银 → 参数回测 → 邮件/微信推送 → 公开部署）；目标 **V0.72.0 = 90 / 100 A-**。
...
| 3 | **CI/CD**：GitHub Actions 自动跑 pytest + ruff，tag 触发自动构建 | ✅ V0.67.0 | 质量门禁（Python 3.11/3.12 matrix + uv 缓存 + concurrency 取消旧 PR + `fail-fast: false` + pytest + ruff check/format + JS 门禁；P1 #1-6 全部闭环） |
